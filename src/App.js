...
async function fsSet(c,i,d){if(!DB)return;await FS.setDoc(FS.doc(DB,c,i),d);}
async function fsDel(c,i){if(!DB)return;await FS.deleteDoc(FS.doc(DB,c,i));}
async function fsUpd(c,i,d){if(!DB)return;await FS.updateDoc(FS.doc(DB,c,i),d);}
function fsListen(c,cb){if(!DB)return()=>{};return FS.onSnapshot(FS.collection(DB,c),s=>cb(s.docs.map(d=>({...d.data(),id:d.id}))));}
...
function scheduleEventReminder(ev,mins){
  ...
  const fire=new Date(new Date(y,mo-1,d,h,parseInt(p.minute)).getTime()-mins*60000);
      ...
      <div ref={ref} onScroll={onScroll} style={{height:"100%",overflowY:"scroll",scrollSnapType:"y mandatory",paddingTop:40,paddingBottom:40,scrollbarWidth:"none"}}>
        {items.map(item=>(
          <div key={item} onClick={()=>onChange(item)}
            style={{height:itemH,display:"flex",alignItems:"center",justifyContent:"center",scrollSnapAlign:"center",fontSize:18,fontWeight:item===String(value)?900:500,color:item===String(value)?"#FF6B35":"#AAA",cursor:"pointer",transition:"all .15s"}}>
            {item}
          </div>
        ))}
      </div>
...
function WeatherBadge({date,location,userCoords}){
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      if(!coords)return;
      const w=await fetchWeather(coords.lat,coords.lon,date);
      if(w)setWeather(w);
    }
    load();
      ...
      {!showPin?(
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{display:"flex",gap:10,justifyContent:"center",marginBottom:16}}>
            {[0,1,2,3].map(i=>(
              <div key={i} style={{width:16,height:16,borderRadius:"50%",background:pinInput.length>i?(pinError?"#EF4444":"#FF6B35"):"rgba(255,255,255,.3)",transition:"all .2s"}}/>
            ))}
          </div>
  ...
  function pressDigit(d){
    if(step==="enter"){
      ...
      if(next.length===4){
        ...
        else{setError("PINs don't match");setConfirm("");setTimeout(()=>{setError("");setStep("set");setInput("");},1200);}
      }
    }
  }
  const dots=step==="confirm"?confirm:input;
  const label=step==="enter"?"Enter PIN":step==="set"?"Set a 4-digit PIN":"Confirm PIN";
  return(
    <div className="mbg">
      <div className="mod fi" style={{textAlign:"center"}}>
        ...
        <p style={{fontSize:13,color:"#888",fontWeight:700,marginBottom:20}}>{label}</p>
        <div style={{display:"flex",gap:12,justifyContent:"center",marginBottom:16}}>
          {[0,1,2,3].map(i=>(
            <div key={i} style={{width:18,height:18,borderRadius:"50%",background:dots.length>i?(error?"#EF4444":"#FF6B35"):"#EEE",transition:"all .2s"}}/>
          ))}
        </div>
...
function EventForm({initial,members,onSave,onCancel,title,userCoords}){
  ...
  return(
    <div>
      ...
      <div style={{background:"#FFF8F0",borderRadius:14,padding:"12px 14px",marginBottom:14}}>
        <TimePicker value={ev.time} onChange={t=>setEv(p=>({...p,time:t}))}/>
      </div>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
        {REMINDER_OPTIONS.map(r=>(
          <button key={r.mins} onClick={()=>setEv(p=>({...p,reminderMins:r.mins}))}
            style={{padding:"7px 12px",borderRadius:20,border:"none",background:(ev.reminderMins??0)===r.mins?"#FF6B35":"#F0F0F0",color:(ev.reminderMins??0)===r.mins?"white":"#666",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>
            {r.label}
          </button>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
        {REPEAT_OPTIONS.map(r=>(
          <button key={r} onClick={()=>setEv(p=>({...p,repeat:r,repeatDays:[]}))}
            style={{padding:"7px 12px",borderRadius:20,border:"none",background:ev.repeat===r?"#FF6B35":"#F0F0F0",color:ev.repeat===r?"white":"#666",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>
            {rLabel[r]}
          </button>
        ))}
      </div>
      {ev.repeat==="daily"&&(
        <div style={{marginBottom:14}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {WEEK_DAYS.map((d,i)=>{
              const checked=(ev.repeatDays||[]).includes(i);
              return(<button key={d} onClick={()=>toggleDay(i)} style={{width:42,height:42,borderRadius:"50%",border:"none",background:checked?"#FF6B35":"#F0F0F0",color:checked?"white":"#666",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>{d}</button>);
            })}
          </div>
        </div>
      )}
...
export default function FamilyHub(){
  ...
  useEffect(()=>{
    ...
    loadFirebase().then(ok=>{
      ...
      FS.getDocs(FS.collection(DB,"members")).then(snap=>{if(snap.empty)DEFAULT_MEMBERS.forEach(m=>fsSet("members",m.id,m));});
  ...
  async function enableNotifications(){
    ...
    if(granted){sendNotif("🏠 Family Hub","Notifications are on!");scheduleDailyChore(choreTime);}
  }
  ...
  async function saveEvent(ev){
    if(!ev.title)return;
    const isEdit=events.some(e=>e.id===ev.id);
    const id=ev.id||`ev_${Date.now()}`;
    const data={id,title:ev.title,time:ev.time||"",date:ev.date||selectedDate,memberId:ev.memberId||null,repeat:ev.repeat||"none",repeatDays:ev.repeatDays||[],reminderMins:ev.reminderMins||0,location:ev.location||""};
    if(fbReady){await fsSet("events",id,data);flash(isEdit?"Event updated ☁️":"Event saved ☁️");}
    else{if(isEdit)setEvents(p=>p.map(e=>e.id===id?data:e));else setEvents(p=>[...p,data]);}
    if(data.reminderMins>0)scheduleEventReminder(data,data.reminderMins);
    setShowEventForm(false);setEditingEvent(null);
  }

  async function deleteEvent(id){
    if(scheduledTimeouts[id]){clearTimeout(scheduledTimeouts[id]);delete scheduledTimeouts[id];}
  ...
  async function addTask(){
    ...
    setNewTask({text:"",memberId:"",stars:1});setShowTaskModal(false);
  }
    ...
    <div style={{fontFamily:"'Nunito',sans-serif",minHeight:"100vh",background:"#FFF8F0"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .mbg{position:fixed;inset:0;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;z-index:2000;padding:16px;}
        .mod{background:white;border-radius:20px;padding:24px;width:100%;max-width:440px;box-shadow:0 20px 60px rgba(0,0,0,.2);max-height:92vh;overflow-y:auto;}
      ...
      {showNotifBanner&&(
        <div style={{background:"#FFF3CD",padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
          ...
          <div style={{display:"flex",gap:8,flexShrink:0}}>
            <button onClick={()=>setShowNotifBanner(false)} style={{background:"none",border:"none",fontSize:12,color:"#AAA",cursor:"pointer",fontWeight:700,fontFamily:"'Nunito',sans-serif"}}>Later</button>
            <button onClick={enableNotifications} style={{background:"#FF6B35",color:"white",border:"none",borderRadius:10,padding:"6px 14px",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>Enable</button>
          </div>
        </div>
      )}
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              {parentPin&&(
                <button onClick={()=>{if(locked){setPinModalMode("unlock");setShowPinModal(true);}else{setLocked(true);flash("App locked 🔒");}}}
                  ...
                  {locked?<span className="lock-pulse">🔒</span>:"🔓"}
                </button>
              )}
              <div style={{textAlign:"right"}}>
                ...
                <div style={{fontSize:10,color:"rgba(255,255,255,.8)",fontWeight:700,marginTop:1,display:"flex",alignItems:"center",justifyContent:"flex-end",gap:4}}>
                  <span style={{width:7,height:7,borderRadius:"50%",background:fbStatus==="live"?"#4ADE80":"#FCD34D",display:"inline-block"}}/>
                  {fbStatus==="live"?"Live":"Demo"}
                  <span style={{marginLeft:2}}>{notifStatus==="granted"?"🔔":"🔕"}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {locked&&(
        <div style={{background:"#FEF2F2",padding:"8px 16px",textAlign:"center",fontSize:13,fontWeight:800,color:"#EF4444",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          ...
          <button onClick={()=>{setPinModalMode("unlock");setShowPinModal(true);}} style={{background:"#EF4444",color:"white",border:"none",borderRadius:8,padding:"3px 10px",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>Unlock</button>
        </div>
      )}
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"16px 12px 100px"}}>
          ...
          <div className="fi">
            ...
            <div style={{display:"flex",gap:6,marginBottom:12,overflowX:"auto",paddingBottom:4}}>
              {[-2,-1,0,1,2].map(off=>{
                ...
                const y=d.getFullYear();const m=d.getMonth();const active=mKey(y,m)===viewMKey;
                return(<button key={off} onClick={()=>{setCalYear(y);setCalMonth(m);}} style={{padding:"6px 12px",borderRadius:12,border:"none",background:active?"#FF6B35":off<0?"#F0F0F0":off===0?"#FFE8D6":"#ECFDF5",color:active?"white":off<0?"#888":off===0?"#FF6B35":"#2D6A4F",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif",whiteSpace:"nowrap",flexShrink:0}}>
            ...
            <div style={{background:isPast?"#F3F4F6":isFuture?"#ECFDF5":"#FFF3E8",borderRadius:14,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
              <div style={{flex:1,fontSize:13,color:"#666",fontWeight:600,fontStyle:monthNoteText?"normal":"italic"}}>{monthNoteText||(isPast?"No notes for this month.":isFuture?"Tap to add planning notes…":"Add notes or goals for this month…")}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
              ...
              {Array.from({length:dCount}).map((_,i)=>{
                const day=i+1;const ds=fmt(calYear,calMonth,day);const evs=eventsFor(ds);const isToday=ds===todayStr;
                return(
                  <div key={day} className="dc" onClick={()=>openDayModal(ds)}
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:2,justifyContent:"center"}}>
                      {evs.slice(0,3).map((ev,ei)=>{const mem=ev.memberId?getMember(ev.memberId):null;return<div key={ei} style={{width:7,height:7,borderRadius:"50%",background:mem?mem.color:"#CCC"}}/>;})}</div>
                  </div>
                );
              })}
            </div>
            <div style={{marginTop:18}}>
              ...
              {expandedEvents.sort((a,b)=>a.date.localeCompare(b.date)).map((ev,ei)=>{
                ...
                return(
                  <div key={`${ev.id}_${ei}`} className="hl" style={{background:"white",borderRadius:14,padding:"11px 14px",marginBottom:7,display:"flex",alignItems:"center",gap:10,boxShadow:"0 2px 8px rgba(0,0,0,.06)",borderLeft:`4px solid ${mem?mem.color:"#CCC"}`}}>
                    ...
                    {!locked&&<button className="eb" onClick={()=>openEditEventForm(events.find(e=>e.id===ev.id)||ev)}>✏️</button>}
                    {!locked&&<button className="xb" onClick={()=>deleteEvent(ev.id)}>✕</button>}
                  </div>
                );
              })}
        ...
        {activeTab==="tasks"&&(
          <div className="fi">
            ...
            <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}}>
              ...
              {members.map(m=>(<button key={m.id} onClick={()=>setFilterMember(filterMember===m.id?null:m.id)} style={pill(filterMember===m.id,m.color)}>{m.emoji} {m.name}</button>))}
            </div>
            {filteredTasks.map(t=>{
              ...
              return(
                <div key={t.id} className="hl" style={{background:"white",borderRadius:16,padding:"13px 15px",marginBottom:8,display:"flex",alignItems:"center",gap:12,boxShadow:"0 2px 8px rgba(0,0,0,.06)",opacity:t.done?0.6:1}}>
                  ...
                  {!locked&&<button className="xb" onClick={()=>deleteTask(t.id)}>✕</button>}
                </div>
              );
            })}
          ...
          <div className="fi">
            <div style={{display:"flex",gap:8,marginBottom:14,background:"white",borderRadius:16,padding:6,boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
              {[{id:"grocery",label:"🛒 Grocery"},{id:"todo",label:"✅ To-Do"}].map(l=>(
                <button key={l.id} onClick={()=>setActiveList(l.id)} style={{flex:1,padding:"10px",borderRadius:12,border:"none",background:activeList===l.id?"#FF6B35":"transparent",color:activeList===l.id?"white":"#888",fontWeight:800,fontSize:14,cursor:"pointer",fontFamily:"'Nunito',sans-serif",transition:"all .2s"}}>{l.label}</button>
              ))}
            </div>
            {!locked&&(
              <div style={{display:"flex",gap:8,marginBottom:12}}>
                <input value={newListItem[activeList]} onChange={e=>setNewListItem(p=>({...p,[activeList]:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addListItem(activeList)} placeholder={`Add to ${activeList==="grocery"?"grocery":"to-do"} list…`} style={S.inp}/>
        ...
        {activeTab==="meals"&&(
          <div className="fi">
            ...
            {Array.from({length:7}).map((_,i)=>{
              ...
              const ds=fmt(d.getFullYear(),d.getMonth(),d.getDate());const dm=meals[ds];const isToday=ds===todayStr;
              return(
                <div key={ds} style={{background:isToday?"#FFF3E8":"white",borderRadius:18,marginBottom:10,overflow:"hidden",boxShadow:"0 2px 10px rgba(0,0,0,.07)",border:isToday?"2px solid #FF6B35":"2px solid transparent"}}>
                  <div style={{padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div>
                      <span style={{fontFamily:"'Fredoka One',cursive",fontSize:15,color:isToday?"#FF6B35":"#333"}}>{DAYS[d.getDay()]} · {MONTHS[d.getMonth()].slice(0,3)} {d.getDate()}</span>
                    ...
                    {!locked&&<button onClick={()=>openMealModal(ds)} style={{background:"#FFE8D6",border:"none",borderRadius:10,padding:"5px 11px",color:"#FF6B35",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>{dm?"Edit":"+ Plan"}</button>}
                  </div>
                  {dm&&(
                    <div style={{padding:"0 16px 14px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
                      {MEAL_SLOTS.map(slot=>dm[slot]&&(<div key={slot} style={{background:"#FFF8F0",borderRadius:10,padding:"7px 10px"}}><div style={{fontSize:9,fontWeight:800,color:"#FF8C42",marginBottom:2}}>{slot.toUpperCase()}</div><div style={{fontSize:13,fontWeight:700,color:"#333"}}>{dm[slot]}</div></div>))}
                    </div>
                  )}
                </div>
              );
        ...
        {activeTab==="family"&&(
          <div className="fi">
            ...
            <div style={{background:locked?"#FEF2F2":"#F0FDF4",borderRadius:16,padding:16,marginBottom:12}}>
              ...
              {!parentPin?(
                <div>
                  ...
                  <button onClick={()=>{setPinModalMode("setup");setShowPinModal(true);}} style={{...S.save,flex:"none",padding:"10px 20px"}}>Set PIN & Lock</button>
                </div>
              ):(
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {locked
                    ?<button onClick={()=>{setPinModalMode("unlock");setShowPinModal(true);}} style={{...S.save,flex:"none",padding:"9px 18px",background:"#EF4444"}}>🔓 Unlock</button>
                    :<button onClick={()=>{setLocked(true);flash("App locked 🔒");}} style={{...S.save,flex:"none",padding:"9px 18px"}}>🔒 Lock Now</button>
                  }
                  <button onClick={()=>{setPinModalMode("change");setShowPinModal(true);}} style={{...S.cancel,flex:"none",padding:"9px 18px"}}>Change PIN</button>
                </div>
              )}
            </div>
            ...
            <div style={{padding:13,background:fbStatus==="live"?"#ECFDF5":"#FFF3E8",borderRadius:14,fontSize:13,fontWeight:700,textAlign:"center",color:fbStatus==="live"?"#2D6A4F":"#856404"}}>
          ...
          <div className="mod fi" onClick={e=>e.stopPropagation()}>
            ...
            {eventsFor(selectedDate).length>0&&(
              <div style={{marginBottom:14}}>
                ...
                {eventsFor(selectedDate).map((ev,ei)=>{
                  const mem=ev.memberId?getMember(ev.memberId):null;
                  return(
                    <div key={`${ev.id}_${ei}`} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",background:"#FFF8F0",borderRadius:12,marginBottom:5,borderLeft:`3px solid ${mem?mem.color:"#CCC"}`}}>
                      <span style={{fontSize:18}}>{mem?mem.emoji:"📅"}</span>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,fontSize:14}}>{ev.title}{repeatBadge(ev)}{reminderBadge(ev)}</div>
                        <div style={{fontSize:12,color:"#888",display:"flex",alignItems:"center",gap:4}}>{ev.time&&<span>{ev.time}</span>}{mem&&<span>· {mem.name}</span>}<WeatherBadge date={ev.date} location={ev.location} userCoords={userCoords}/></div>
                      </div>
                      {!locked&&<button className="eb" onClick={()=>openEditEventForm(events.find(e=>e.id===ev.id)||ev)}>✏️</button>}
                      {!locked&&<button className="xb" onClick={()=>deleteEvent(ev.id)}>✕</button>}
                    </div>
      ...
      {showEventForm&&editingEvent&&(
        <div className="mbg" onClick={()=>{setShowEventForm(false);setEditingEvent(null);}}>
          <div className="mod fi" onClick={e=>e.stopPropagation()}>
            <EventForm initial={editingEvent} members={members} title={events.some(e=>e.id===editingEvent.id)?"✏️ Edit Event":"📅 New Event"} userCoords={userCoords} onSave={saveEvent} onCancel={()=>{setShowEventForm(false);setEditingEvent(null);}}/>
          </div>
        </div>
      )}
      ...
      {showMonthPlanner&&(
        <div className="mbg" onClick={()=>setShowMonthPlanner(false)}>
          <div className="mod fi" onClick={e=>e.stopPropagation()}>
            <h3 style={{fontFamily:"'Fredoka One',cursive",fontSize:21,color:isPast?"#6B7280":"#FF6B35",marginBottom:4}}>{isPast?"📜 Month Record":isFuture?"🗓️ Plan Ahead":"📝 Month Notes"}</h3>
            <p style={{fontSize:13,color:"#888",fontWeight:700,marginBottom:14}}>{MONTHS[calMonth]} {calYear}</p>
            <textarea value={monthInput} onChange={e=>setMonthInput(e.target.value)} readOnly={isPast} placeholder={isFuture?"Goals, trips, dates to prepare for…":"Notes, goals, or highlights…"} rows={6} style={{width:"100%",padding:"12px 14px",borderRadius:14,border:"2px solid #FFE0C8",fontSize:14,fontWeight:600,outline:"none",resize:"vertical",background:isPast?"#FAFAFA":"white"}}/>
        ...
        <div className="mbg" onClick={()=>setShowTaskModal(false)}>
          <div className="mod fi" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {[1,2,3].map(n=>(<button key={n} onClick={()=>setNewTask(p=>({...p,stars:n}))} style={{flex:1,padding:"9px",borderRadius:12,border:"none",background:newTask.stars===n?"#FFD700":"#F5F5F5",fontWeight:800,fontSize:16,cursor:"pointer"}}>{"⭐".repeat(n)}</button>))}
              </div>
            </div>
            <div style={{display:"flex",gap:8}}>
      ...
      {showMealModal&&(
        <div className="mbg" onClick={()=>setShowMealModal(false)}>
          <div className="mod fi" onClick={e=>e.stopPropagation()}>
            ...
            {MEAL_SLOTS.map(slot=>(<div key={slot} style={{marginBottom:9}}><label style={{fontSize:11,fontWeight:800,color:"#FF8C42",display:"block",marginBottom:4}}>{slot.toUpperCase()}</label><input value={mealInput[slot]||""} onChange={e=>setMealInput(p=>({...p,[slot]:e.target.value}))} placeholder={`What's for ${slot.toLowerCase()}?`} style={S.inp}/></div>))}
        ...
        <div className="mbg" onClick={()=>setEditingMember(null)}>
          <div className="mod fi" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14,background:"#FFF8F0",borderRadius:16,padding:14,marginBottom:18,borderLeft:`5px solid ${editingMember.color}`}}>
              ...
              <div><div style={{fontWeight:900,fontSize:18}}>{editingMember.name||"Name…"}</div><div style={{fontSize:12,color:editingMember.color,fontWeight:700}}>⭐ {starMap[editingMember.id]||0} stars</div></div>
            </div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4,background:"#FFF8F0",borderRadius:12,padding:10,marginBottom:16}}>
              {EMOJI_OPTIONS.map(em=>(<button key={em} className="eopt" onClick={()=>setEditingMember(p=>({...p,emoji:em}))} style={{background:editingMember.emoji===em?"#FFE8D6":"transparent",border:editingMember.emoji===em?"2px solid #FF6B35":"2px solid transparent"}}>{em}</button>))}
            </div>
            <label style={LBL}>COLOR</label>
            <div style={{display:"flex",flexWrap:"wrap",gap:9,marginBottom:20}}>
              {COLOR_PALETTE.map(c=>(<div key={c} className="cdot" onClick={()=>setEditingMember(p=>({...p,color:c}))} style={{background:c,border:editingMember.color===c?"3px solid #111":"3px solid transparent",transform:editingMember.color===c?"scale(1.3)":"scale(1)"}}/>))}
            </div>
            <div style={{display:"flex",gap:8}}>
